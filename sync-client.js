...
function getBaseURLs(){
    ...
    return {http:'http://'+host+':3000',https:'https://'+host+':3001'};
}
async function probeServer(url){
    try{var r=await fetch(url+'/ip',{signal:AbortSignal.timeout(3000)});var d=await r.json();if(d.ok&&d.ip){return url.replace(/\/\/[^:\/]+/,'//' + d.ip);}}
    catch(e){}
    return null;
}
async function setupPrincipal(name){
    _name=name;_key=_key||makeKey();
    var urls=getBaseURLs();console.log('[SYNC] Buscando servidor...');
    var found=await probeServer(urls.https);
    if(!found) found=await probeServer(urls.http);
    if(found){_url=found;}else{_url=urls.http;}
    console.log('[SYNC] URL: '+_url);
    localStorage.setItem('jam_sync_url',_url);localStorage.setItem('jam_sync_name',_name);localStorage.setItem('jam_sync_key',_key);
    _active=true;if(_timer)clearInterval(_timer);_timer=setInterval(bidirectionalSync,30000);
...
function scanAndConnect(){
    return new Promise(function(resolve,reject){
        ...
        videoEl.style.cssText='width:400px;height:400px;object-fit:cover;border-radius:12px;border:3px solid #8b5cf6;background:#111';
        var hint=document.createElement('div');hint.style.cssText='color:#fff;font-size:14px;margin-top:12px;text-align:center';hint.textContent='Preparando camara...';
        ...
        function beginScan(constraints){
            navigator.mediaDevices.getUserMedia(constraints)
            .then(function(s){
                stream=s;videoEl.srcObject=s;
                videoEl.onloadedmetadata=function(){
                    videoEl.play().then(function(){
                        hint.textContent='Escaneando... apunta al QR';
                        ...
                        console.log('[SCAN] Play error:',e.message);
                        hint.textContent='Error al iniciar camara. Toca para reintentar.';
                        videoEl.onclick=function(){videoEl.onclick=null;beginScan(constraints);};
                    });
                };
            })
            .catch(function(e){
                console.log('[SCAN] Camera error:',e.message);
                if(constraints.video && constraints.video.facingMode && constraints.video.facingMode.ideal){
                    console.log('[SCAN] Retrying without facingMode...');
                    beginScan({video:true});
                }else{
                    cleanup();scanFallbackFile().then(resolve).catch(reject);
                }
            });
        }
        beginScan({video:{facingMode:{ideal:'environment'},width:{ideal:1280},height:{ideal:720}}});
    });
}
...
async function bidirectionalSync(){
    ...
    var getDatos=window.jamGetAllDatos;var combinar=window.jamCombinarImportacion;var saveIDB=window.jamSaveIDB;
    if(!getDatos||!combinar)return{ok:false,msg:'App no lista'};
    _syncing=true;console.log('[SYNC] Iniciando sync con '+_url);
    try{
        var localData=await getDatos();
        console.log('[SYNC] Enviando: '+(localData.productos||[]).length+' productos, '+(localData.ventas||[]).length+' ventas');
        ...
        var serverResult=await r.json();console.log('[SYNC] Servidor:',JSON.stringify(serverResult));
        ...
        console.log('[SYNC] Recibido: '+(serverStore.productos||[]).length+' productos');
        var d=window.D;if(!d)throw new Error('window.D no disponible');
        var totalAdded=0,totalUpdated=0,totalSkipped=0;
        for(var j=0;j<STORES.length;j++){
            var st=STORES[j];if(!serverStore[st]||!Array.isArray(serverStore[st]))continue;
            var campoNombre=st==='productos'?'nombre':null;
            var result=combinar(d[st]||[],serverStore[st],campoNombre);
            d[st]=result.lista;totalAdded+=result.agregados;totalUpdated+=result.actualizados||0;totalSkipped+=result.omitidos;
            console.log('[SYNC] '+st+': '+result.lista.length+' (+'+result.agregados+' new, ~'+(result.actualizados||0)+' upd)');
        ...
        var msg='Sync OK: '+totalAdded+' nuevos, '+totalUpdated+' actualizados';console.log('[SYNC] '+msg);
        if(notify)notify(msg,'success');_syncing=false;
        return{ok:true,added:totalAdded,updated:totalUpdated,skipped:totalSkipped};
    }catch(e){
        _syncing=false;console.log('[SYNC] ERROR: '+e.name+': '+e.message);
        var msg2='Sync: ';
        if(e.name==='TypeError')msg2+='No conecta con '+_url+'. Verifica que node sync-server.js este corriendo.';
        else if(e.name==='AbortError')msg2+='Timeout. Servidor lento.';
        else msg2+=e.message;
        var notify2=typeof mostrarNotificacion==='function'?mostrarNotificacion:null;
        if(notify2)notify2(msg2,'warning');return{ok:false,msg:msg2};
    }
}
function startSync(){if(!_url)return;_active=true;if(_timer)clearInterval(_timer);_timer=setInterval(bidirectionalSync,30000);}
function stopSync(){_active=false;if(_timer){clearInterval(_timer);_timer=null;}}
function tryAutoReconnect(){
    if(!_url||!_name){return Promise.resolve(false);}
    console.log('[SYNC] Auto-reconnect: '+_url);
    return fetch(_url+'/ip',{signal:AbortSignal.timeout(4000)}).then(function(r){return r.json();}).then(function(d){
        if(d.ok){_active=true;if(_timer)clearInterval(_timer);_timer=setInterval(bidirectionalSync,30000);console.log('[SYNC] Reconectado');return true;}
        return false;
